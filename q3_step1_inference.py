"""
Q3 Step 1 Inference: Human Causality Stance — Mention Detection
===============================================================
Runs inference on Q1-filtered tweets to detect whether each tweet makes
any explicit statement about whether climate change is caused by humans.

Input:  Q1 inference results (tweets with Q1 label 1 or 2)
Output: Same rows + binary prediction (Mentioned / Not Mentioned)

Only "Mentioned" tweets proceed to Q3 Step 2 for stance classification.
...
MODEL_PATH      = "/kaggle/input/your-model/q3_step1_fine_tuned_model"
INPUT_DATA_PATH = "/kaggle/input/inference-added/inference_4_q1_done_2025-10-18.csv"
OUTPUT_PATH     = "/kaggle/working/q3_step1_inference_results.csv"
...
def load_q1_filtered_tweets(filepath: str, text_column: str = 'title') -> pd.DataFrame:
    ...
    Args:
        filepath:    Path to Q1 inference output CSV
        text_column: Name of the tweet text column

    Returns:
        Filtered DataFrame (Q1 label in {1, 2})
    """
    ...
    # Filter to Q1 labels 1 and 2 (climate-relevant tweets)
    if 'q1_prediction_index' in df.columns:
        df_filtered = df[df['q1_prediction_index'].isin([1, 2])].copy()
        print(f"  After Q1 filter (labels 1 & 2): {len(df_filtered):,} tweets")
    else:
...
def run_inference(model, tokenizer, texts: list, batch_size: int, device: str):
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader, desc="Q3 Step 1"):
...
def main(
    ...
    print("=" * 70)
    print("Q3 STEP 1 INFERENCE: Human Causality — Mention Detection")
    print("=" * 70)
    ...
    # Attach results
    df['q3_step1_prediction_numeric'] = predictions
    df['q3_step1_prediction']         = [LABEL_MAP[p] for p in predictions]
    df['q3_step1_prob_not_mentioned'] = probabilities[:, 0]
    df['q3_step1_prob_mentioned']     = probabilities[:, 1]
    df['q3_step1_confidence']         = probabilities.max(axis=1)
    ...
    print(f"\nMean confidence: {df['q3_step1_confidence'].mean():.4f}")
    ...
    print("\n" + "=" * 70)
    print("Q3 STEP 1 INFERENCE COMPLETE!")
    print("=" * 70)
...
if __name__ == "__main__":
    ...
    # =========================================================================
    # 1. Update MODEL_PATH to your trained Q3 Step 1 model
    # 2. Update INPUT_DATA_PATH to your Q1 inference output CSV
    # 3. Verify text column name (default: 'title')
    # 4. Output will contain only Q1-relevant tweets (~360k)
    # 5. Only "Mentioned" rows proceed to q3_step2_inference.py
    # =========================================================================
